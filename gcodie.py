...
def generate_layer_img(layer, x, y, z, output_dir, bg_color, layer_color, image_size=(800, 800)):
...
def generate_multiple_layers(count, x, y, z, output_dir, bg_color, layer_color, image_size=(800, 800)):
    os.makedirs(output_dir, exist_ok=True)
    layers = int(np.max(z) / 0.2) + 1  # Adjust the height step as needed

    if count > layers:
        print(f"Error: Requested count ({count}) exceeds the total number of layers ({layers}).")
        return
    ...
    try:
        for layer in tqdm(range(count), desc='Images Processed', unit='image', colour='green'):
            ...
            try:
                ...
                print(f"Error: Invalid background color '{bg_color}'. Defaulting to white.")
        ...
        print(f"{count - skip}/{count} images saved. {skip} layers skipped due to no points.")
